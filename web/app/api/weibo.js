...
const BASE_URL = "http://localhost:3000/api"; // 这里要填写你的后端API地址
...
export const getTimeId = async (timestamp) => {
	...
	const response = await axios.get(
		`${BASE_URL}/time_id/${formattedTimestamp}`
	);
	return response.data;
};
...
export const getTimeIdAndAllData = async (timestamp) => {
	...
	if (timeIdResult && timeIdResult.data) {
		...
		if (time_id) {
			...
			const allDataResult = await axios.get(
				`${BASE_URL}/all_data_list/${time_id}`
			);
